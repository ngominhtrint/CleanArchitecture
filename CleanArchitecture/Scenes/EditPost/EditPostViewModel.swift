...
final class EditPostViewModel: ViewModelType {
    ...
    func transform(input: EditPostViewModel.Input) -> EditPostViewModel.Output {
        let errorTracker = ErrorTracker()
        let editing = input.editTrigger.scan(false) { editing, _ in
            return !editing
        }.startWith(false)
        
        let saveTrigger = editing.skip(1) // do not need initial state
            .filter { $0 == false }
            .mapToVoid()
        ...
        let post = Driver.combineLatest(Driver.just(self.post), titleAndDetails) { (post, titleAndDetails) -> Post in
            return Post.init(body: titleAndDetails.1, title: titleAndDetails.0, uid: post.uid, userId: post.userId, createdAt:  post.createdAt)
        }.startWith(self.post)
        ...
        return Output.init(editButtonTitle: editButtonTitle,
                           save: savePost,
                           delete: deletePost,
                           editing: editing,
                           post: post,
